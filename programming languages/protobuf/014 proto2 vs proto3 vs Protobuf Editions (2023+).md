...
- [ ] Arena allocation: `google::protobuf::Arena`, зачем нужен, как ускоряет аллокации для больших графов сообщений.
- [ ] Move-семантика в сгенерированном коде, `Swap()`, избегание лишних копий.
- [ ] Reflection API (`google::protobuf::Message::GetReflection()`) — для generic-кода, работающего с произвольными типами сообщений.
- [ ] `Any`, `Timestamp`, `Duration`, `Struct`, `Empty` из `google/protobuf/*.proto` (well-known types).
- [ ] Практика: переписать программу дня 2 с использованием Arena и сравнить бенчмарк аллокаций.
