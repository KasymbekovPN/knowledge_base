...
    std::shared_ptr<Book> book0 {
	    std::make_shared<Ebook>("About C++", 420, 7)
	};
    std::shared_ptr<Ebook> ebook0 {
	    std::dynamic_pointer_cast<Ebook>(book0)
    };
    printEbook(ebook0);

    std::shared_ptr<Book> book1 {std::make_shared<Book>("About go", 421)};
    std::shared_ptr<Ebook> ebook1 {
	    std::dynamic_pointer_cast<Ebook>(book1)
	};
    printEbook(ebook1);
...
```
{title: About C++, pages: 420, size: 7}
The object is not Ebook
```
