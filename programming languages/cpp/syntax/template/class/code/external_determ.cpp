...
template<class T> Person<T>::Person(T id, std::string name) noexcept: id{id}, name{name} {}
...
template<class T> Person<T>& Person<T>::operator=(const Person<T>& other) noexcept{
    if (this != &other) {
        id = other.id;
        name = other.name;
    }
