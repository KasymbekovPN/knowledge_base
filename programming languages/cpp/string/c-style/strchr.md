...
`std::strchr(const char *str, int ch)` ищет первый символ ch в строке str. Возвращает указатель на первый символ или nullptr.
...
    const char *found = strchr(original, 'w');
    _test_result(found, original);

    const char *not_found = strchr(original, 'a');
