...
- **Плюсы**: Не создает новых элементов, не бросает исключений
- **Минусы**: Требует проверки итератора
...
### 4. Метод `contains()` (C++20)
```cpp
if (umap.contains("banana")) {
    // Безопасный доступ, так как ключ существует
    int bananas = umap["banana"];
}
```
...
### Безопасный доступ с проверкой (C++17)
```cpp
...
```cpp
// Через operator[]
umap["apple"] += 2;  // Увеличит значение на 2

// Через итератор
auto it = umap.find("banana");
if (it != umap.end()) {
    it->second = 5;  // Изменяем значение
}
```



Пример идиоматичного доступа (C++20):
```cpp
std::unordered_map<std::string, int> inventory = {{"sword", 1}, {"potion", 5}};

// Проверка и доступ
if (inventory.contains("potion")) {
    std::cout << "Potions: " << inventory["potion"] << '\n';
}

// Модификация с проверкой
if (auto it = inventory.find("sword"); it != inventory.end()) {
    it->second += 1;  // Увеличиваем количество мечей
}
```
...
```cpp
// Через operator[] (создает элемент, если не существует)
int val = map1["apple"];

// Через at() (бросает исключение, если не существует)
try {
    int val = map1.at("apple");
