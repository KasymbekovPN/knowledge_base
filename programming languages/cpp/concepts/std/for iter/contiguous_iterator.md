...
`std::contiguous_iterator` — самый “сильный” iterator concept в C++20.

Он означает:
> элементы расположены непрерывно в памяти (contiguous memory).
...
- `random_access_iterator`: можно быстро прыгать по элементам, __НО__ память может быть fragmented.
-  `contiguous_iterator` гарантирует: элементы лежат подряд как массив

# Реальные contiguous iterators
...
потому что хранится блоками памяти.

# Для чего нужен contiguous_iterator

Позволяет STL делать оптимизации:
- memcpy
- SIMD
- bulk operations
- zero-copy access
