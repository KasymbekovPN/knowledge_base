...
Планировщик (scheduler) — это то, что превращает разрозненные корутины в работающую кооперативную многозадачность: он держит очередь готовых к выполнению корутин и прокручивает их по очереди. Здесь сойдётся всё пройденное — handle, `await_suspend` с возвратом `void`, очередь готовых задач.

Идея: корутина через `co_await scheduler.schedule()` добровольно уступает управление — кладёт свой handle в очередь планировщика и приостанавливается. Планировщик в цикле достаёт handle'ы из очереди и возобновляет их. Это ровно кооперативная модель из самого первого пункта: переключение только в явных точках (`co_await`), никакого вытеснения.
...
```
slow tick 0 @ 65119ms
[external thread] injecting a task at ~50ms
>>> latecomer woke the loop and ran!
slow tick 1 @ 65431ms
slow tick 2 @ 65740ms
slow tick 3 @ 66045ms
slow tick 4 @ 66348ms
...
По плану закрыт этап 4 в его ключевой части (генератор, Task, scheduler — всё собрано и работает). Дальше:

- **Этап 5** — готовые инструменты: `std::generator` (C++23), cppcoro, корутины в Boost.Asio. Это уже не «как устроено», а «что брать в проде».
- **Этап 7** — подводные камни: висячие ссылки в лямбдах-корутинах, время жизни захватов, аллокации.

Что интереснее — посмотреть, как тот же `Generator`/`Task` выглядит в реальных библиотеках (этап 5), или разобрать ловушки времени жизни на конкретных примерах с UB (этап 7)?
