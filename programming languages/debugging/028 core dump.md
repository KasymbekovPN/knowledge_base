...
Core dump — это **read-only снимок**: нельзя `continue`, `step`, `next` (программа физически больше не выполняется — процесса нет, есть только его "фотография" памяти на момент смерти). Можно только **исследовать** состояние (переменные, стек, память), но не управлять исполнением. Для этого и существует связка "локально воспроизвёл → нашёл баг вживую" vs "получил core dump из прода → диагностировал постфактум, без возможности再re-run".
...
Хочешь дальше — **Reverse debugging в GDB** (`record`, `reverse-step`, `reverse-continue`), следующий пункт плана?
