...
Хочешь, продолжим по плану дальше — Этап 2 (breakpoints/watchpoints), или сначала закрепим `RelWithDebInfo` на практике: соберём наш `coro_demo.cpp` с `-O2 -g` и посмотрим, насколько хуже станет видимость переменных по сравнению с `-Og`?
...
## Этап 1. Основы: сборка и первый запуск (1-2 дня)

- 
- Запуск: `gdb ./binary`, `lldb ./binary`, передача аргументов и переменных окружения
- Attach к работающему процессу (`gdb -p PID`, `lldb -p PID`)
- Основные различия в философии команд GDB vs LLDB (GDB — текстовые команды, LLDB — команды в стиле Xcode + совместимость с GDB через алиасы)

## Этап 2. Точки останова и управление выполнением (2-3 дня)

- Breakpoints: по функции, файлу:строке, условные breakpoints (`break if x > 5`)
