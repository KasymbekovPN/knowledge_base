...
Хорошо, tokio собрался. Начнём с самого важного — написать свой `Future` вручную, чтобы увидеть механику голыми руками (по аналогии с твоими custom awaiter'ами в C++20).Работает. Теперь проверим, что `.await` действительно не блокирует другие задачи (конкурентность через `tokio::spawn`):Именно так: все три стартуют сразу, финишируют в порядке своего `sleep`, а не порядка запуска — как и ожидалось от кооперативного планировщика. Теперь классическая ловушка — блокировка `std::sync::Mutex` внутри async-кода:Отлично — компилятор поймал эту проблему ещё до запуска! Покажем механику: `MutexGuard` из `std::sync` — не `Send`, а раз он "жив" в момент `.await`, он становится частью сгенерированной state machine, которая должна быть `Send` для `tokio::spawn`. Зафиксируем это и покажем исправление через `tokio::sync::Mutex`:Работает корректно и последовательно. Теперь `tokio::select!` и async mpsc-каналы:Раз у тебя уже плотная база по C++20 корутинам (promise_type, coroutine_handle, awaiter-механика, symmetric transfer, Boost.Asio) — буду сравнивать напрямую, без разжёвывания азов корутин с нуля.
...
Хочешь закрыть `Stream` (асинхронный аналог `Iterator`, тоже напрямую связан с тем, что уже знаешь) — или это была последняя тема из твоего изначального плана, и стоит вместо этого перейти к практике: собрать что-то цельное (например, простой async TCP-сервис на tokio, раз у тебя уже есть опыт с async TCP echo-серверами на Boost.Asio для прямого сравнения)?
