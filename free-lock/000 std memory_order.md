...
В стандарте C++ (начиная с C++11 и включая актуальные C++20/C++23) в перечислении `std::memory_order` определено 6 возможных значений. Каждое из них указывает компилятору и процессору, насколько строго можно переупорядочивать инструкции чтения и записи вокруг атомарной операции.

Ниже приведена сводная таблица всех значений, расположенных в порядке от самого слабого к самому строгому:

| Значение `std::memory_order` | Тип операции                        | Описание и влияние на процессор/компилятор                                                                                                                                                                                                                                   | Главная сфера применения                                                                            |
| ---------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `memory_order_relaxed` [1]   | Любая (Load, Store, RMW)            | Самый слабый режим. Гарантирует только атомарность (ячейка памяти не прочитается наполовину). Компилятор и процессор могут свободно переупорядочивать любые другие инструкции вокруг неё. Нет никаких барьеров памяти.                                                       | Атомарные счетчики (инкремент статистики, `shared_ptr` ref count), флаги остановки потоков.         |
| `memory_order_consume` [1]   | Чтение (Load) [1]                   | Ограничивает переупорядочивание только для тех инструкций, которые напрямую зависят по данным от считанного значения. Архитектурно сложен, компиляторы временно приравнивают его к `acquire`. Не рекомендуется к использованию в современном коде.                           | Исторически — чтение указателей в структурах типа RCU (Read-Copy-Update) [1] в ядре Linux.          |
| `memory_order_acquire` [1]   | Чтение (Load) [1]                   | Захват. Ни одна инструкция чтения или записи, находящаяся в коде _после_ этой операции, не может быть перенесена компилятором или процессором _до_ неё. Синхронизируется с `release`-записью в другом потоке.                                                                | Вход в критическую секцию (`Spinlock::lock`), чтение флага готовности данных.                       |
| `memory_order_release` [1]   | Запись (Store) [1]                  | Освобождение. Ни одна инструкция чтения или записи, находящаяся в коде _до_ этой операции, не может быть перенесена компилятором или процессором _после_ неё. Все изменения в памяти, сделанные до этого, гарантированно станут видны потоку, выполнившему `acquire`-чтение. | Выход из критической секции (`Spinlock::unlock`), публикация указателя на готовые данные.           |
| `memory_order_acq_rel` [1]   | Чтение-Модификация-Запись (RMW) [1] | Комбинированный режим. Действует как `acquire` для фазы чтения и как `release` для фазы записи этой операции. Поток одновременно «видит» всё, что было до него, и «публикует» свои изменения для тех, кто будет после.                                                       | Атомарные операции над очередями и стеками (например, `fetch_add`, `exchange`, `compare_exchange`). |
| `memory_order_seq_cst` [1]   | Любая (Load, Store, RMW)            | Последовательная согласованность. Самый строгий режим (используется по умолчанию). Накладывает барьеры `acq_rel`, а также гарантирует единый глобальный порядок всех `seq_cst` операций для всей системы. Процессор жестко сбрасывает все буферы записи.                     | Безопасный код «по умолчанию», где критически важно избежать любых тонких эффектов многопоточности. |

## Важное архитектурное правило C++

Некоторые модели памяти аппаратно привязаны к направлению движения данных, поэтому их нельзя передавать некорректным операциям:

- Передать `memory_order_release` в метод `.load()` — это ошибка (вызовет UB или провалит `assert` в отладочной сборке стандартной библиотеки).
- Передать `memory_order_acquire` в метод `.store()` — это ошибка.
- В операциях RMW (например, `compare_exchange_strong`) можно передавать две модели: одну для случая успеха (например, `acq_rel`), вторую для случая неудачи (например, `acquire` или `relaxed`).

Если вам интересно, мы можем посмотреть, во что превращается `memory_order_relaxed` по сравнению с `memory_order_seq_cst` при обычном `load()` (чтении) в ассемблере x86-64 — вы увидите, почему `seq_cst` для чтения на этой платформе обходится дороже. Хотите взглянуть на разницу в ASM?
