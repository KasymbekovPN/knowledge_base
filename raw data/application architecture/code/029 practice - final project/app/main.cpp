...
namespace {
    ...
    void loadSeedTasks(domain::TaskRepository& repo, const std::string& path) {
        ...
        if (!file) {
            std::cerr << std::format("Could not open: {}, skip seed\n", path);
            return;
        }
...
int main(int argc, char *argv[]) {
    const std::string CONFIG_PATH{
        argc > 1
        ? argv[1]
        : R"(C:\projects\knowledge_base\raw data\application architecture\code\029 practice - final project\config\app.cfg)"
    };
    const std::string SEED_PATH{
        argc > 2
        ? argv[2]
        : R"(C:\projects\knowledge_base\raw data\application architecture\code\029 practice - final project\config\seed_tasks.csv)"
    };
    ...
    loadSeedTasks(repo, SEED_PATH); // View уже отрисовала стартовое состояние сама
