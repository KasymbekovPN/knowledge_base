...
// Mediator / Event bus. Единственная зависимость Model -> "наружу" - и то
// только на этот заголовок, а не на кого-то конкретного (View, EventLoop
// и т.д. друг про друга вообще не знают).

// ВНИМАНИЕ: не потокобезопасна (см. обсуждение в предыдущем вопросе про
// `auto listCopy = it->second` и TSan-репродукцию гонки в handlers_).
// Всё приложение сознательно однопоточное - весь ввод идёт через один
// EventLoop, как договорились в самом начале (Day 1: single-threaded loop).
class EventBus {
public:
    ...
    void publish(const EventT& event) {
        ...
        // защита от реентерабельной мутации, см. ASan-демо
